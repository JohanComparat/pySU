...
f=open(join("outputs", "outputs-m200c-cen-fits-all-at-once.txt"),'w')
...
n.savetxt(join("outputs","M200c-cen-cumulative-function-best_fit.txt"),n.transpose([n.hstack((X)), n.hstack((Y)), n.hstack((Z))]) )

#######################################################
# now plots the results of the fit
print "now plots the results of the fit"

vmax_mod, z_mod, n_mod = n.loadtxt(join("outputs","M200c-cen-cumulative-function-best_fit.txt"), unpack=True)
...
p.ylim((-9,0))
p.grid()
p.savefig(join("outputs","M200c-cen-cumulative-function-model.png"))
p.clf()
...
p.ylim((-9,0))
p.grid()
p.savefig(join("outputs","M200c-cen-cumulative-function-data.png"))
p.clf()
...
p.legend(loc=3)
p.grid()
p.savefig(join("outputs","M200c-cen-cumulative-function-chi2pp.png"))
p.clf()
