...
p.xlabel('f sat')
p.ylabel('M mean')
p.plot( bestfit[2], bestfit[0], 'k*',mec='w',ms=20,alpha=0.2)
...
index = n.argmin(chi2map[4])
bestfit = chi2map.T[index]
...
p.ylabel('M mean')
p.plot( bestfit[2], bestfit[0], 'k*',mec='w',ms=20,alpha=0.2)
p.scatter(chi2map[2], chi2map[0],c=chi2map[4],s=60)
cb=p.colorbar()
cb.set_label('chi2 / ndof')
p.savefig(join(data_clustering_dir,"Fischer-griw-Mmean-fsat-xiOnly.pdf"))
p.clf()
    
index = n.argmin(chi2map[3])
bestfit = chi2map.T[index]
...
p.ylabel('M mean')
p.plot( bestfit[2], bestfit[0], 'k*',mec='w',ms=20,alpha=0.2)
p.scatter(chi2map[2], chi2map[0],c=chi2map[3],s=60)
cb=p.colorbar()
cb.set_label('chi2 / ndof')
p.savefig(join(data_clustering_dir,"Fischer-griw-Mmean-fsat-wOnly.pdf"))
p.clf()
