...
type = "hist"
cos = "Satellite" #"Central" # centrak or satellite ?
qty = "vmax"

f=open(join("outputs_vmax", "outputs-vmax-sat-fits-all-at-once.txt"),'w')
...
n.savetxt(join("outputs_vmax","vmax-sat-cumulative-function-best_fit.txt"),n.transpose([n.hstack((X)), n.hstack((Y)), n.hstack((Z))]) )

#######################################################
# now plots the results of the fit
print "now plots the results of the fit"

vmax_mod, z_mod, n_mod = n.loadtxt(join("outputs_vmax","vmax-sat-cumulative-function-best_fit.txt"), unpack=True)
...
p.ylim((-7,0))
p.grid()
p.savefig(join("outputs_vmax","vmax-sat-cumulative-function-model.png"))
p.clf()
...
p.ylim((-7,0))
p.grid()
p.savefig(join("outputs_vmax","vmax-sat-cumulative-function-data.png"))
p.clf()
...
p.legend(loc=3)
p.grid()
p.savefig(join("outputs_vmax","vmax-sat-cumulative-function-chi2pp.png"))
p.clf()
