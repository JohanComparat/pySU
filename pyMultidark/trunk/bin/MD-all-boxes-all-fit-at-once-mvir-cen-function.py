...
f=open(join("outputs_mvir", "outputs-mvir-cen-fits-all-at-once.txt"),'w')
...
chi2fun = lambda ps : n.sum( (vfGbis(mvir,redshift,ps) - yData)**2. / (yData*0.035)**2. )/(len(yData) - 8)
...
pOpt = res.x
cov = res.direc
chi2perpoint = lambda ps : (vfGbis(mvir,redshift,ps) - yData)**2. / (yData*0.035)**2. 
...
xModel = n.arange(n.min(mvir),15,0.1)
X,Y = n.meshgrid(xModel,n.arange(0,n.max(redshift)+0.025,0.025))
Z = vfGbis(X,Y,pOpt)

n.savetxt(join("outputs_mvir","mvir-cen-cumulative-function-best_fit.txt"),n.transpose([n.hstack((X)), n.hstack((Y)), n.hstack((Z))]) )

#######################################################
# now plots the results of the fit
print "now plots the results of the fit"

vmax_mod, z_mod, n_mod = n.loadtxt(join("outputs_mvir","mvir-cen-cumulative-function-best_fit.txt"), unpack=True)
...
p.ylim((-9,0))
p.grid()
p.savefig(join("outputs_mvir","mvir-cen-cumulative-function-model.png"))
p.clf()
...
p.ylim((-9,0))
p.grid()
p.savefig(join("outputs_mvir","mvir-cen-cumulative-function-data.png"))
p.clf()
...
p.legend(loc=3)
p.grid()
p.savefig(join("outputs_mvir","mvir-cen-cumulative-function-chi2pp.png"))
p.clf()
