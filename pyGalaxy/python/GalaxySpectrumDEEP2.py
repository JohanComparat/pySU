...
class GalaxySpectrumDEEP2:
	...
	def __init__(self,catalog_entry,calibration=True,lineFits=False):
		...
		self.deep2_dir = self.database_dir+"DEEP2/"
		self.deep2_catalog_dir = self.deep2_dir+"catalogs/"
		self.deep2_spectra_dir = self.deep2_dir+"spectra/"

		if calibration :
			self.path_to_spectrum = glob.glob(self.deep2_spectra_dir + self.mask +'/*/*' + self.objno+'*.fits')

		if lineFits :
			self.path_to_spectrum = glob.glob(self.deep2_spectra_dir + self.mask +'/*/*' + self.objno+'*_fc.dat')
