	...
	for i in range(len(ZQgrid)-1):
		for j in range(len(ZCgrid)-1):
			for k in range(len(Zgrid)-1):
				sel = (table['PLATE']>6000) & (table['zQ']>ZQgrid[i])&(table['zQ']<ZQgrid[i+1]) & (table['zCont']>ZCgrid[j])&(table['zCont']<ZCgrid[j+1]) & (table['Z']>Zgrid[k])&(table['Z']<Zgrid[k+1])# &(table['Z_1']>0)&(table['Z_1']>table['Z_ERR_1'])&(table['Z_ERR_1']>0)
