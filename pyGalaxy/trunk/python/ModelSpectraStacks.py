...
class ModelSpectraStacks:
	...
	def __init__(self, stack_file, cosmo=cosmo, firefly_min_wavelength= 1000., firefly_max_wavelength=7500., dV=-9999.99, N_spectra_limitFraction=0.8):
		...
		self.redshift = float(self.stack_file.split('_')[1].split('-')[-1][1:])
		...
		self.selection = (flA>0) & (self.hdu1.data['NspectraPerPixel']  > float( self.stack_file.split('_')[-6]) * self.N_spectra_limitFraction )
