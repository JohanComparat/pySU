...
from firefly_constants import constants
...
def spectrum_velocity_scale(wave, log10=False):
    ...
    return ac.c.to('km/s').value*dl_over_l
...
class spectral_resolution:
    ...
    def adjusted_resolution(self, indx=None):
        ...
        """
        if indx is None:
            return 1.0/np.sqrt( np.square(self.cnst.sig2fwhm/self.c)            * self._convert_pd2vd(self.sig_pd*np.absolute(self.sig_pd))            + 1.0/np.square(self.sres()) )

        return 1.0/np.sqrt( np.square(self.cnst.sig2fwhm/self.c)     * (self._convert_pd2vd(self.sig_pd*np.absolute(self.sig_pd)))[indx]        + 1.0/np.square(self.sres()[indx]) )
...
def log_rebin_pix(lamRange, n, oversample=None, velscale=None, log10=False, newRange=None):
    ...
    c = ac.c.to('km/s').value    # Speed of light in km/s (use astropy definition)
