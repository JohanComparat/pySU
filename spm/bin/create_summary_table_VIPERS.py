...
init_cat=join(os.environ['VIPERS_DIR'], "catalogs", "VIPERS_W14_summary_v2.1.linesFitted.fits")
plate_catalog = join(os.environ['VIPERS_DIR'], "catalogs", "VIPERS_W14_summary_v2.1.linesFitted.spm.fits")
hdu_orig_table = fits.open(init_cat)
orig_table = hdu_orig_table[1].data
orig_cols = orig_table.columns

kroupaFolder = join( os.environ['VIPERS_DIR'], 'stellarpop-m11-kroupa', 'stellarpop')
salpeterFolder = join( os.environ['VIPERS_DIR'], 'stellarpop-m11-salpeter', 'stellarpop')
...
for catalog_entry in orig_table:
	krF = join(kroupaFolder, 'spFly-vipers-'+catalog_entry['id_IAU'][7:]+"-kr.fits")
	ssF = join(salpeterFolder, 'spFly-vipers-'+catalog_entry['id_IAU'][7:]+"-ss.fits")
