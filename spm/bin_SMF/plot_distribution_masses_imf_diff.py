...
def plotDIFF(imf_ref, imf_1, imf_2, m_bins = n.arange(-10., 10., 0.1)):
	...
	p.xlim((-7.5, 7.5))
	p.grid()
	p.savefig(os.path.join(out_dir, "delta_M_distribution_eboss"+imf_ref.split('_')[1]+".png" ))
	p.clf()
	# normalized comparison ratio
	...
	p.xlim((-7.5, 7.5))
	p.grid()
	p.savefig(os.path.join(out_dir, "delta_A_distribution_eboss"+imf_ref.split('_')[1]+".png" ))
	p.clf()
	# normalized comparison ratio
	...
	p.hist(delta_m1[bad==False], bins=m_bins, histtype='step', label=imf_1.split('_')[1]+"-"+imf_ref.split('_')[0] , normed=True )
	bad = (delta_m2==-n.inf)|(delta_m2==n.inf)|n.isnan(delta_m2)
	p.hist(delta_m2[bad==False], bins=m_bins, histtype='step', label=imf_2.split('_')[1]+"-"+imf_ref.split('_')[0] , normed=True )
	p.plot(m_bins, norm.pdf(m_bins, loc=0, scale=1), label='N(0,1)', ls='dashed')
	p.ylabel('normed distribution')
	...
	p.xlim((-7.5, 7.5))
	p.grid()
	p.savefig(os.path.join(out_dir, "delta_Z_distribution_eboss"+imf_ref.split('_')[1]+".png" ))
	p.clf()
