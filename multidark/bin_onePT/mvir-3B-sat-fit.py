...
n.savetxt(join(os.environ['MULTIDARK_LIGHTCONE_DIR'], qty,"mvirFunction_sat_parameters_ST01_MFonly_fit.txt"), n.transpose([pOpt_ST01, pErr_ST01]), header="A a p")
...
n.savetxt(join(os.environ['MULTIDARK_LIGHTCONE_DIR'], qty,"mvirFunction_sat_parameters_BA11_MFonly_fit.txt"), n.transpose([pOpt, pCov.diagonal()**0.5]), header="A a p q")

def plotSel(MDsel, label):
	x_data = logsig[ok & MDsel]
	y_data = log_MF[ok & MDsel]
	y_data_err = (data["std90_pc_cen"][ok & MDsel]**2. + data["dN_counts_cen"][ok & MDsel]**(-1.))**(0.5) + systErr
...
#p.yscale('log')
p.grid()
p.savefig(join(os.environ['MULTIDARK_LIGHTCONE_DIR'], qty,"fit-BA11-"+cos+"-differential-function-residual.png"))
p.clf()
...
def plotAll(MDsel, label):
	...
	y_data_err = (data["std90_pc_cen"][ok2 & MDsel]**2. + data["dN_counts_cen"][ok2 & MDsel]**(-1.))**(0.5) + systErr
	y_model = log_fsigma(x_data, pOpt[0], pOpt[1], pOpt[2], pOpt[3])
	if len(x_data)>0:
...
#p.yscale('log')
p.grid()
p.savefig(join(os.environ['MULTIDARK_LIGHTCONE_DIR'], qty,"fit-BA11-"+cos+"-differential-function-residual-incompleteness.png"))
p.clf()
...
p.grid()
p.savefig(join(os.environ['MULTIDARK_LIGHTCONE_DIR'], 'mvir',"covariance","p_parameter_constrain.png"))
p.clf()
...
p.ylabel(r'$\chi^2/ndof$')
p.grid()
p.savefig(join(os.environ['MULTIDARK_LIGHTCONE_DIR'], 'mvir',"covariance","q_parameter_constrain.png"))
p.clf()
